...
const CHEVRON = icon('<path d="M6 9l6 6 6-6"/>')
...
export const workCss = `
  #work {
    ...
    background: var(--paper); border: 1px solid var(--hairline); box-shadow: 0 6px 24px rgba(0, 0, 0, 0.12);
  ...
  #work .strip { display: flex; align-items: center; gap: 8px; overflow-x: auto; padding: 2px 0; min-width: 0; }
  #work .chip {
...
export function createWorkIsland(
  ...
  const rebuild = (): void => {
    ...
    strip.append(...pins.filter(pin => pin.status !== 'Done').map(pin => chip(pin, onSelect)))
    if (finished.length > 0) {
      const doneLabel = document.createElement('span')
      doneLabel.className = 'done-label'
      doneLabel.textContent = 'Done'
      strip.append(doneLabel, ...finished.map(pin => chip(pin, onSelect)))
    }
    island.append(
      label,
