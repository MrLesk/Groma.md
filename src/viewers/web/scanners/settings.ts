...
import { scannerSettingLabel, type ScannerSetting, type ScannerSettings, type ScannerSettingsAction } from '../../../scanner/modules/settings-model.ts'
...
export const scannerSettingsCss = `
  ...
  #scanner-settings button:disabled { opacity: .5; cursor: wait; }
  ...
  #scanner-settings .scanner-match { overflow-wrap: anywhere; max-width: 260px; }
...
function settingRow(scanner: ScannerSetting): string {
  ...
  const primary = scanner.source ? (scanner.status === 'missing' ? button('restore', 'Restore') : button('remove', 'Remove')) : (scanner.installSource ? button('install', 'Install') : '')
  const more = scanner.source ? button('update', 'Update') : ''
  return `<tr><td><strong>${id}</strong><div class="scanner-origin">${scanner.official ? 'Official' : 'Third-party'}${scanner.version ? ` · ${escaped(scanner.version)}` : ''}</div><details><summary>Details</summary><p>${escaped(scanner.source ?? scanner.name)}</p><p>${escaped(scanner.message)}</p>${more}</details></td>`
    + `<td class="scanner-match">${escaped(scanner.matches.join('\n') || (scanner.match === 'unknown' ? 'Match unknown' : 'No matching files'))}</td>`
...
export function bindScannerSettings(data: WebDataSource): void {
  ...
  dialog.innerHTML = '<header><h1 id="scanner-settings-title">Scanners</h1><button type="button" data-add>Add scanner</button><button type="button" data-check>Check again</button><button type="button" data-close aria-label="Close scanner settings">×</button></header>'
  ...
  function paintRows(next: ScannerSettings): void {
    ...
    rows.innerHTML = next.scanners.length ? `<table><thead><tr><th>Scanner</th><th>Project match</th><th>Status</th><th></th></tr></thead><tbody>${next.scanners.map(settingRow).join('')}</tbody></table>` : (next.notice.tone === 'error' ? '' : '<p>No source project detected. Add a scanner to this project.</p>')
  ...
  function paint(next: ScannerSettings): void {
    ...
    control!.innerHTML = tone === 'hint' ? `● Scanners<span class="scanner-hint"> · ${next.notice.message.startsWith('More') ? 'More available' : 'Check needed'}</span>` : 'Scanners'
    ...
    paintRows(next)
    for (const button of dialog.querySelectorAll('button')) button.disabled = busy && !button.hasAttribute('data-close')
  ...
  dialog.querySelector('[data-check]')!.addEventListener('click', () => { void change({ action: 'check' }) })
  rows.addEventListener('click', event => {
    ...
    if (action === 'update') sourceForm(id)
