...
import { layerLabelsSvg, layerPlanesSvg } from './layer-planes.ts'
...
import { buildingsSvg, facadeDefs } from './buildings.ts'
import { islandsSvg, sheetSvg, slabsSvg } from './ground.ts'
import { routesSvg } from './routes.ts'
import { createGrid } from '../grid.ts'
import { DEFAULT_PROJECTION } from '../projection/project.ts'
import { weightAt } from './scale.ts'
import { HOVERABLE, mapDefs, patternAttributes } from './style.ts'
...
export interface IsoMap {
  ...
  /** Navigation is about to head for `destination`; a zoom-out draws it first, as the camera layer explains. */
