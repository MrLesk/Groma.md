...
/** Columns kept free on each side of the map so neighbouring islands or containers peek in. */
...
export function rootIslands(model: TerminalViewModel): RootIsland[] {
  ...
  const islands = [...model.sheet.islands]
    .sort((left, right) => left.rect.gx - right.rect.gx || left.rect.gy - right.rect.gy)
...
/** The selectable root stops, island by island: the system itself, then its rows. */
export function rootStops(model: TerminalViewModel): string[][] {
  return rootIslands(model)
    .map(entry => [
      ...(entry.island.element === null ? [] : [entry.island.element.representationId]),
      ...entry.rows.map(row => row.element.representationId),
    ])
}

/** The first container row of the first internal system shown on the root map. */
export function firstRootRow(model: TerminalViewModel): AnnotatedElement | undefined {
  return rootIslands(model)
    .find(entry => {
      return entry.island.kind === 'system'
        && entry.rows.length > 0
    })?.rows[0]?.element
}
...
/**
 * Islands west to east, each listing one row per child: kind glyph and name, then one block per
 * component wrapped inside the island; an island never exceeds the map minus the padding and grows down.
 */
