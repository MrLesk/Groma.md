...
In an interactive terminal, bare `groma` reconstructs the same self-contained bundle as
`groma export` and opens it from a temporary local file. The embedded web client receives one
finite snapshot from bounded shared reads; it owns only disposable folding, focus, and selection
state and performs no canonical write or network request.
