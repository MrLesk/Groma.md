...
Both plugins share authored flow membership, relationship captions, sibling
order and the containment tree from `src/viewers/` and `src/element-order.ts`.
A relationship describes a standing collaboration. A `Groma Flow` record
explains one scenario with ordered From, To, and Action steps. Each step
references exactly one existing directed relationship; omitted connections
never join the flow. A selected flow and step are viewer state. The record
belongs to the architecture model but adds no C4 element or geometry.
