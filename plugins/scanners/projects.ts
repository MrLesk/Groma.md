...
/**
 * The package directories that depend on a framework, keep its TypeScript config where `options` allows, and hold a
 * source with one of `extensions`, among the files `excluded` leaves in.
 */
export async function frameworkProjects(
  ...
  const files = await repositoryFiles(root, file => !excluded(file))
