...
/** Keep Vue's compatible compiler isolated from Groma's embedded TypeScript SDK. */
export async function buildPackage(destination: string): Promise<void> {
  ...
  const result = await Bun.build({
    ...
    plugins: [{ name: 'vue-typescript', setup(build) {
  ...
  await writeFile(path.join(destination, 'package.json'), `${JSON.stringify({
    ...
    groma: { scanner: { id: 'vue', entry: './dist/index.js' } },
  }, null, 2)}\n`)
  await cp(path.join(root, '../../../LICENSE'), path.join(destination, 'LICENSE'))
  await cp(path.join(root, '../../../docs/scanners/vue/index.md'), path.join(destination, 'README.md'))
  await cp(path.join(root, '../../../docs/scanners/vue/validation.md'), path.join(destination, 'validation.md'))
}
