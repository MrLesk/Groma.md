...
/** The package directories whose React source the scanner reads, including those that use an ancestor config. */
export function reactProjects(root: string): Promise<string[]> {
  return frameworkProjects(root, 'react', ['.tsx'], { inheritConfig: true })
}

/**
 * Visit every React package one at a time, so its compiler program is released before the next loads. Each
 * reads the TypeScript scanner's files less the project's exclusions.
 */
async function eachProject(root: string, excluded: (file: string) => boolean,
  ...
  const directories = await reactProjects(root)
...
export async function checkReactReadiness(root: string): Promise<void> {
  await eachProject(root, () => false, async () => {})
}
...
export async function scanReact(root: string, _settings?: ScannerSettings, excluded: (file: string) => boolean = () => false) {
  ...
  await eachProject(root, excluded, async (directory, project) => {
    const observation = await scanProject(root, directory, project)
