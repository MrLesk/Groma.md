...
public final class Main {
    ...
    public static void main(String[] args) {
        try {
            if (args.length != 3) throw new IllegalArgumentException("Expected repository root, release and classpath");
            ...
            var files = new BufferedReader(new InputStreamReader(System.in, StandardCharsets.UTF_8))
                .lines().filter(line -> !line.isEmpty()).map(root::resolve).toList();
    ...
    private static Object analyze(Path root, List<Path> files, String release, String classpath) throws Exception {
        ...
        try (var manager = compiler.getStandardFileManager(diagnostics, Locale.ROOT, StandardCharsets.UTF_8)) {
            ...
            var task = (JavacTask) compiler.getTask(compilerOutput, manager, diagnostics,
                List.of("-proc:none", "-implicit:none", "--release", release, "-encoding", "UTF-8", "-Xlint:none"),
            ...
            return Json.object(
                ...
                "root", Json.object("kind", "java-source-set", "name", root.getFileName().toString(), "file", "groma-java.json"),
