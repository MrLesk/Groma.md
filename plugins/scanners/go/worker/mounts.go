...
// mount is the router another router is mounted on, and the path under it. An unknown mount
// silences its routes, because their served path is longer than the source states here.
type mount struct {
	source   *source
	receiver ast.Expr
...
// router's own prefix. A receiver this scan cannot read serves under a path it cannot state.
// Only a chi router routes below its mount: any other router still reads the full URL, which
// its own routes do not state, so it reports nothing.
func (e *evidence) mountedRouter(object types.Object, known router) (router, bool) {
	under, mounted := e.mounted[object]
	if !mounted {
		return known, true
	}
	if under.unknown || known.framework != chi {
		return router{}, false
	}
	...
	return router{framework: known.framework, prefix: prefix}, true
}
