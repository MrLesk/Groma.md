...
Simplicity round:
7. The draft row says containers and components require --parent, matching src/draft.ts.
<!-- SECTION:PLAN:END -->
...
Simplicity round: the structure guide's groma draft row now says containers and components require --parent <id>, as src/draft.ts enforces, instead of saying they take it. bun run check in an isolated worktree at 70fb5408 with only this change passed: 16 node tests, 589 bun tests pass (35 skipped), 0 fail; biome warnings only in other files.
<!-- SECTION:NOTES:END -->
...
Simplicity round: the draft row says containers and components require --parent; verified with bun run check in an isolated worktree.
