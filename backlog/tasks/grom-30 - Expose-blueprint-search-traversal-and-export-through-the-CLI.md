...
Exposed deterministic, bounded blueprint export, search, and relationship traversal through shared Application Operations and the CLI. Published identity-pinned groma.graph-query/v2 and groma.operations/v2 surfaces, kept legacy v1 initialization-only, made export items self-contained and restart-safe, added actionable aggregate-bound diagnostics, and raised atomic CLI rendering to eight MiB so an official limit-one export derived from a valid one-MiB intent document remains renderable. Sealed workspace operation access behind a same-source frozen v2 facade with Promise and diagnostic containment. Reconciled the official 256-character search boundary and 1–16 traversal-depth boundary while keeping Application provider-neutral, and preserved exact engine-owned character, term, and depth maxima through bounded Application-owned diagnostics. Verified with bun run check: 736 tests and 5,286 assertions plus format, typecheck, architecture boundaries, build, smoke, compiled workflow, and crash recovery; bun run check:targets passed all four targets; final specification review PASS; final quality review APPROVED; git diff --check clean.
