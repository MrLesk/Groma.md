...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Moving the details cursor over a person's commands lights that command's walk on the map immediately, scoped to that person
- [ ] #2 Enter commits the browsed command, so the walk stays lit after leaving the details pane
- [ ] #3 Leaving details without Enter (Esc, Left, or selecting elsewhere) drops the preview and restores the committed walk, or no walk when none was committed
- [ ] #4 The details pane still marks the committed command distinctly from the browsed one, and x clears the committed walk
- [ ] #5 The previewed-versus-committed walk derivation is covered by fixture tests and bun test passes
<!-- AC:END -->
