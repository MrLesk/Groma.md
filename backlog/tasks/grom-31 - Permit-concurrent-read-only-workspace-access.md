...
Release sequencing decision: Iteration 2 intentionally supports the single-active-CLI-process first-run path. Concurrent independent read processes remain a known fail-closed limitation owned by GROM-31 in Iteration 3; an overlap may be rejected, but it must never corrupt or partially rewrite canonical state. Iteration 2 release documentation must not claim that concurrent reads work.
<!-- SECTION:NOTES:END -->
