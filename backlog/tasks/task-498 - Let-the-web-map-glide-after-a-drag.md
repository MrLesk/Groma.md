...
4. Tests (authority: TASK-498 AC1-AC3). web-camera-motion.test.ts: a glide moves in the release direction, covers less ground each interval, stops, never moves for a release slower than the stop speed, and a move replaces it (wrong results caught: a glide that never stops, reverses, or keeps going after navigation; existing tests cover only transitions and framing). web-map-pointer.test.ts: a release while moving asks for a glide at the drag's speed, and a paused release, a tap and a pinch do not (wrong results: gliding after a pause or pinch, wrong velocity; existing tests cover only pinch and tap). Existing expectations gain the hold call.
