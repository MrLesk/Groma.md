...
Independent Codex audit remediation: project publication now returns only one private frozen opaque witness through the replaceable coordination provider, retains the semantic Result solely in the owned closure, requires exact outer witness identity, and then validates the captured settlement. A malicious provider regression awaits stale update/remove callbacks and attempts failure-to-success rewrites; both operations retain project-revision-conflict and exact configuration bytes. Host lifecycle now defaults to semantic-required recovery and exposes one explicit management-not-required policy; runProgram selects it only through an exact package/project command-kind allowlist. Configuration status remains validated, the surface receives recovery not-required, and requireWorkspace stays closed. Lifecycle spies prove configured management skips recover while ordinary default startup recovers. CLI integration proves project list plus lockless package disable/remove remain available with malformed canonical intent while component roots fails closed with workspace-recovery-failed and preserves intent bytes. Added proof-only scanner GraphData U+2028/U+2029 parse/serialize/parse stable-byte regression without serializer changes. Verification: focused Host/CLI suite 253 pass; full check 983 pass plus build, smoke, Iteration 1A/crash recovery, Iteration 1B, and self-blueprint workflows; check:targets passed Darwin ARM64, Linux x64, Windows x64, and Windows ARM64; formatting, typecheck, architecture boundaries, and diff hygiene passed.

Final coordination quality remediation: replaced the project-registry coordination callback with a strict one-shot admission state machine. Only one publication can be admitted; sequential or concurrent reentry and retained-callback use never publish and force a fail-closed unsettled result. The registry closes admission when the outer coordinator settles, fences and awaits an admitted publication even if the outer returns early, and accepts semantic settlement only when exactly one execution completed without failure or misuse and the outer success contains the exact private primary witness. Misuse is unavailable before publication could change state and indeterminate afterward; thrown or rejected pre/post-publication paths remain contained. Added regressions for pre-publication rejection, sequential/concurrent double invocation (at most one commit and never ordinary success/conflict), early outer return, retained callback after settlement, and the prior mutable settlement exploit. Verification: dedicated project-registry suite 20 pass; focused Host/CLI suite 257 pass; full bun run check 987 pass plus build, smoke, Iteration 1A/crash recovery, Iteration 1B foundation, and self-blueprint workflows; check:targets passed Darwin ARM64, Linux x64, Windows x64, and Windows ARM64; formatting, typecheck, architecture boundaries, and diff hygiene passed.
<!-- SECTION:NOTES:END -->
