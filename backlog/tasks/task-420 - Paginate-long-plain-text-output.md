...
9. Fix: draft summaries (groma view <draft-id>) ignored --max-count, --skip and --count; route the draft's items through pagedAnswer with the draft identity, outcome and completion as the head.
10. Fix: a cut page printed empty sections as none and the owner-record tail while omitting earlier populated sections. A page becomes the matching slice of the complete answer: an empty section prints on the page that reaches its place, the tail on the page that reaches the end, so consecutive pages joined equal the complete answer. Update the inspect guide and the reassembly test to compare pages in order.
11. Skip: moving the owner-record command after the footer on a cut file answer (optional finding), because AC #3 requires cut output to end with the range, total and next command; the owner command now prints on the last page before the footer.

Simplicity round:
12. The paging tests for plain-world move to test-bun/plain-view.test.ts; the rejoin helper takes the head from the complete answer and checks both pages repeat it. The empty-section condition in pagedAnswer drops its redundant clause. The inspect guide states the paging invariant instead of a second copy of the placement rules. printListPage documents that its closing lines print on every page.
<!-- SECTION:PLAN:END -->
...
External review round (HEAD cf8e7975): draft summaries (groma view <draft-id>) ignored --max-count, --skip and --count; they now page through pagedAnswer, with the draft id and kind, outcome and completion as the head and the touched elements as the Items section, so --count prints the number of items. A cut page also printed empty sections as none and the owner-record tail while it omitted earlier populated sections. The earlier rule that an empty section prints on every page is reversed: a page is now its slice of the complete answer, so the head repeats, an empty section prints on the page that holds the item after it, and the tail prints on the page that holds the last item, just above the footer; one holds(position) predicate decides both. Skipped the optional finding to print the owner command after the footer, because AC #3 requires cut output to end with the footer. The inspect guide Paging paragraph now lists groma view <draft-id> and describes the slice rule. Tests in test-bun/list-window.test.ts: expectPagesRejoin checks that two consecutive pages repeat the head and, joined in order, equal the complete answer, on the viewer-view overview, the plain-view overview split at its empty Flows index, the plain-view shop drill-down ending with an empty section, and the flows src/entry.ts file answer ending with the owner command; a draft test covers --count and a past-the-end page. The three new tests failed against HEAD. Probes: empty answer with --skip, middle boundary, last page and past-the-end page print as described. bun run check in an isolated worktree at cf8e7975 with only these changes passed: 16 node tests, 515 bun tests pass (32 skipped), 0 fail; biome warnings only in other files.

Simplicity round: the plain-world paging tests moved from test-bun/list-window.test.ts to test-bun/plain-view.test.ts, which owns plain-world.ts; expectPagesRejoin takes the head block count from its caller, checks that the second page repeats the complete answer's head, and no longer renders a count or past-the-end page; record() adds --plain to the typed command only for plain answers. pagedAnswer drops the redundant empty-section clause, so a section prints on the pages holding its first position or any of its items. The inspect guide states the invariant (a page repeats what it describes; consecutive pages, read in order, print the complete answer once) instead of repeating the placement rules. printListPage documents that its closing lines print on every page, unlike an answer's tail. bun run check in an isolated worktree at ae7995e4 with only these changes passed: 16 node tests, 589 bun tests pass (35 skipped), 0 fail; biome warnings only in other files.
<!-- SECTION:NOTES:END -->
...
Review round: groma view <draft-id> now pages its items too, so the paged answers are the overview, the drill-down, the draft summary, the source-file answer, groma lint, groma scanner discover and groma scanner list. A cut groma view page is now its slice of the complete answer (replacing the rule that an empty section prints on every page): the head repeats, an empty section prints on the page holding the item after it, and the owner-record command on the page holding the last item, so consecutive pages joined in order equal the complete answer. Verified with new rejoin and draft tests that failed before the fix, and bun run check in an isolated worktree.

Simplicity round: the paging tests for plain answers now live with plain-world's tests, the section rule lost a redundant clause, and the inspect guide states the paging invariant once; verified with bun run check in an isolated worktree.
