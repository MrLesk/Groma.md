...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The package exposes a `groma` executable that resolves through a Bun link to the repository build output
- [x] #2 After one `bun link` setup, rebuilding `dist/groma` changes the executable used by the `groma` command without another link or copy step
- [x] #3 The documented local development loop distinguishes the one-time link setup from each rebuild and from the standalone-copy workflow
- [x] #4 Focused tests cover the linked command resolution behavior
<!-- AC:END -->
