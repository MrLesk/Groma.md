...
AC #6 rerun after the review-fix round (main a8d9ef0e, scanner builds in the shared tree rebuilt from it; the Java build needed its dist cleared first). callforpapers at 1cb6783f3 was cloned into a scratch directory (the owner's working copy was only read), initialized fresh, and given the owner's scanners.json (angular, java, php, typescript, all from the rebuilt dist packages). groma scan created 1229 elements; facts: angular 205 requests, java 433 endpoints and 3 requests, php 12 requests, typescript none.
Derived HTTP rows: 13 now against 14 in the previous AC #6 run (0b4c137e).
Gone (3): admin-schedule-copy.service.ts -> ScheduleCopyResource.java (POST apply and preflight), admin-ticket-allocation.service.ts -> SpeakerResource.java (GET /api/speakers/ticket-allocation), entities/event/event.service.ts -> PublicResource.java (GET /api/public/event). Each calls through a class field such as this.resourceUrl = appConfig.serverApiUrl + '/api/...'; the Angular producer now reports a read through this as a configured base and drops the field's own text, so these requests arrive as configured /preflight, /apply or / and core compares nothing it should.
New (2): entities/wishlist-entry/wishlist-entry.service.ts -> SpeakerResource.java, GET /api/speakers/:id, correct (appConfig.serverApiUrl now resolves to its literal '' and SpeakerResource maps /api/speakers/{id}; ClientForwardController's forwards are constrained and lose to the literal /api). callforpaper/speaker/speaker.service.ts -> UserResource.java, DELETE /api/users/:userId, POST /api/users, PUT /api/users, WRONG: the source calls this.resourceUrl + '/users' with resourceUrl = appConfig.serverApiUrl + '/api/speakers', which SpeakerResource serves at /api/speakers/users; the truncated fact configured /users reaches UserResource's /api/users after removing /api.
Spot check against client calls and Spring mappings: account.service.ts -> AccountResource GET and POST /api/account correct; admin-export.service.ts -> SpeakerResource GET /api/speakers/download/:proposalState correct; cfp-top-talks-home.component.ts -> PublicUserRatingResource GET /api/public/ratings/top correct; wishlist-entry.service.ts -> SpeakerResource GET /api/speakers/:id correct; speaker.service.ts -> UserResource wrong, as above.
Cause, outside the core slice: plugins/scanners/http-values.ts held() returns 'unseen' (a configured part) for any field read through this, which contradicts the owner decision that configured marks only a value read from configuration. Reported to the orchestrator for the TypeScript-family producers; core is unchanged.
<!-- SECTION:NOTES:END -->
