...
Implemented subtraction-first cleanup: 96 changed files, 69,381 lines removed and 1,117 added overall. Production/scripts net -18,843 lines; tests net -48,753 lines; other docs/config and task metadata account for the remainder. Removed local plugin packages and package declarations, schema migration, durable observation recovery, evidence sharding, persistent projection bundles/Merkle/repair/checkpoints, custom executable parsing, the certification benchmark, and frozen self-blueprint/foundation proof suites. Replaced them with built-in-only scanner composition, completed-snapshot-only in-process scan execution, an in-memory reconstructable projection, ordinary same-process validation, and focused compiled verification. One bounded independent audit confirmed the same major seams; no repeated review loop was started. `bun run check` passes: formatting, types, boundaries, 383 tests, native build/smoke, and the compiled Iteration 1A workflow. `bun run check:targets` cross-compiles four targets, runs the host-compatible workflow, and restores the native artifact. The first automatic Codex review found four valid race/refresh issues; the follow-up makes projection rebuild failures expire stale cached state, reserves scan keys before async setup, honors pre-start cancellation, and propagates/races cancellation through completed-snapshot consumption with focused regression tests.
<!-- SECTION:NOTES:END -->
