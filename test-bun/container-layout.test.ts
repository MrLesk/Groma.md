...
function files(count: number, name = 'file'): { scanner: string; file: string; lines: number }[] {
  return Array.from({ length: count }, (_, index) => ({ scanner: 'test', file: `src/${name}${index}.ts`, lines: 10 + index }))
}

test.concurrent('a building lists one row per floor with the largest file and +N; a ghost shows one empty row', () => {
  const model = worldOf([
    box('sys', 'system', CELL, { children: ['observed:store'] }),
    box('store', 'container', CELL, { parent: 'observed:sys', children: ['observed:full', 'observed:ghost'] }),
    box('full', 'component', CELL, { parent: 'observed:store', code: files(7) }),
    box('ghost', 'component', CELL, { parent: 'observed:store', origin: 'draft' }),
  ])
  const projection = projectWorld(model, { viewport: VIEWPORT, level: 'components', currentId: 'observed:full' })
  const full = projection.items.find(item => item.representationId === 'observed:full')!
  const floors = model.sheet.buildings.find(building => building.representationId === 'observed:full')!.floors
  assert.ok(floors.length >= 1 && floors.length <= 5)
  assert.deepEqual(full.lines, floors.map(floor => `${floor.files[0]!.split('/').at(-1)}${floor.files.length > 1 ? ` +${floor.files.length - 1}` : ''}`))
  assert.equal(full.cellBounds.height, full.lines.length + 2)
  assert.equal(full.cellBounds.width, Math.max(full.title.length + 6, ...full.lines.map(row => row.length + 4)))
...
test.concurrent('buildings never overlap and two rows lie between their lines', () => {
  ...
  const cards = projectWorld(model, { viewport: VIEWPORT, level: 'components', currentId: 'observed:part0' }).items
    .filter(item => item.shape === 'card').map(item => item.worldBounds)
...
test.concurrent('a row wider than its building ends in an ellipsis', () => {
  const model = worldOf([
    box('sys', 'system', CELL, { children: ['observed:store'] }),
    box('store', 'container', CELL, { parent: 'observed:sys', children: ['observed:long'] }),
    box('long', 'component', CELL, { parent: 'observed:store', code: files(1, 'a-very-long-source-file-name-that-no-building-can-hold-') }),
  ])
  const projection = projectWorld(model, { viewport: VIEWPORT, level: 'components', currentId: 'observed:long' })
  ...
  const row = [...(rows[card.y + 1] ?? '')].slice(card.x + 2, card.x + card.width - 2).join('').trimEnd()
